    ...
    public class ReadPreferenceServerSelectorTests
    {
        ...
        private IEnumerable<ServerDescription> GetConnectedServers(bool primaryConnected=true, bool secondariesConnected=true, bool includeTagSets = false)
        {
            ...
            servers.Add(ServerDescriptionBuilder.Build(n =>
            {
                if (includeTagSets)
                {
                    n.ReplicaSetInfo(new ReplicaSetInfo("rs", new DnsEndPoint("localhost", 1000), new DnsEndPoint[0], new Dictionary<string, string> { { "a", "true" }, { "b", "true" } }));
                }
            ...
            servers.Add(ServerDescriptionBuilder.Build(n =>
            {
                if (includeTagSets)
                {
                    n.ReplicaSetInfo(new ReplicaSetInfo("rs", new DnsEndPoint("localhost", 1000), new DnsEndPoint[0], new Dictionary<string, string> { { "a", "true" }, { "c", "true" } }));
                }
            ...
            servers.Add(ServerDescriptionBuilder.Build(n =>
            {
                if (includeTagSets)
                {
                    n.ReplicaSetInfo(new ReplicaSetInfo("rs", new DnsEndPoint("localhost", 1000), new DnsEndPoint[0], new Dictionary<string, string> { { "b", "true" }, { "c", "true" } }));
                }
            ...
            servers.Add(ServerDescriptionBuilder.Build(n =>
            {
                if (includeTagSets)
                {
                    n.ReplicaSetInfo(new ReplicaSetInfo("rs", new DnsEndPoint("localhost", 1000), new DnsEndPoint[0], new Dictionary<string, string> { { "b", "true" }, { "d", "true" } }));
                }
