...
namespace MongoDB.Driver.Core.Connections.Security.SaslMechanisms.Gsasl
{
    /// <summary>
    ...
    internal class GsaslSession : SafeHandle
    {
        ...
        public void SetProperty(string name, string value)
        {
            ...
            NativeMethods.gsasl_property_set(this, prop, value);
        }
        ...
        public byte[] Step(byte[] input)
        {
            ...
            try
            {
                ...
                var rc = NativeMethods.gsasl_step(this, inputPtr, input.Length, out outputPtr, out outputLength);
                if (rc != NativeMethods.GSASL_OK && rc != NativeMethods.GSASL_NEEDS_MORE)
                {
                    var message = NativeMethods.GetError(rc);
                    throw new GsaslException(rc, message);
                }

                _isComplete = rc == NativeMethods.GSASL_OK;
            ...
            finally
            {
                ...
                {
                    NativeMethods.gsasl_free(outputPtr);
                }
            }
        }
